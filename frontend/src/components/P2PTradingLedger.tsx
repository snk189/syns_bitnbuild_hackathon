...
export const P2PTradingLedger: React.FC<P2PTradingLedgerProps> = ({
  ...
  const formatParticipantName = (id: string) => {
    if (id.includes("SOLAR_COMMUNITY")) return "☀️ Community Solar";
    if (id.includes("SOLAR_FARM")) return "☀️ Solar Farm";
    if (id.includes("BATT") || id.includes("BATTERY")) return "🔋 Central Battery";
    if (id.includes("PROSUMER")) return "🏡 Prosumers";
    if (id.startsWith("HOUSE_")) return `🏡 House ${id.replace("HOUSE_", "")}`;
    return id;
  };
      ...
      <div className="flex justify-between items-start pb-2.5 mb-2.5 border-b border-slate-800/80">
        <div>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            Neighbors buy excess local clean energy directly from each other at rates cheaper than the utility
          </p>
        </div>
        <div className="flex items-center space-x-2 text-[11px] shrink-0">
          ...
          <span className="text-emerald-400 bg-emerald-950/50 border border-emerald-800/60 px-2 py-0.5 rounded font-mono font-bold">
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="text-[10px] uppercase font-bold text-slate-400 border-b border-slate-800/80 pb-1">
                ...
                <th className="py-1.5 px-2">Seller (Clean Energy)</th>
                <th className="py-1.5 px-2">Buyer (Local Prosumer)</th>
                <th className="py-1.5 px-2 text-right">Energy</th>
                <th className="py-1.5 px-2 text-right">Fair Rate</th>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {trades.slice(-20).reverse().map((t) => {
                const tradeSavings = Math.max(0, t.energy_kwh * (gridRetailTariff - t.price_kwh));
                return (
                  <tr key={t.trade_id} className="hover:bg-slate-900/50 transition-colors">
                    ...
                    <td className="py-2 px-2 text-amber-300 font-semibold text-[11px]">
                      {formatParticipantName(t.seller_id)}
                    </td>
                    <td className="py-2 px-2 text-cyan-300 font-semibold text-[11px]">
                      {formatParticipantName(t.buyer_id)}
                    </td>
                    <td className="py-2 px-2 text-right font-mono font-bold text-slate-200">{t.energy_kwh.toFixed(1)} kWh</td>
                    <td className="py-2 px-2 text-right font-mono text-purple-300 font-semibold">₹{t.price_kwh.toFixed(2)}</td>
                    <td className="py-2 px-2 text-right font-mono text-emerald-400 font-bold">
                      +₹{tradeSavings.toFixed(1)}
                    </td>
